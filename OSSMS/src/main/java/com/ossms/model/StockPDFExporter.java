...
public class StockPDFExporter {
	List<Stocks> sto = new ArrayList<Stocks>();

	public StockPDFExporter(List<Stocks> sto) {
		super();
		this.sto = sto;
	}
	...
	private void writeTableData(PdfPTable table) {
		...
		for(Stocks s : sto) {
			table.addCell(String.valueOf(s.getIdPurchasedStock()));
			table.addCell(String.valueOf(s.getPaymentID()));
			table.addCell(String.valueOf(s.getQuantity()));
			table.addCell(String.valueOf(s.getPurchaseDate()));
	...
	public void export(HttpServletResponse response) throws DocumentException, IOException {
		...
		Paragraph Topic2 = new Paragraph("Purchased Stock Report" , font2);
